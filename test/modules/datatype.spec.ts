  ...
  describe(`random seeded tests for seed ${faker.seed()}`, () => {
    for (let i = 1; i <= NON_SEEDED_BASED_RUN; i++) {
      describe.each([
        ...
        it(`should return a value that will result in \`true\` when checked against JavaScript's \`typeof value === '${type}'\` operator`, () => {
          const actual = faker.datatype[type]();
          expect(actual).toBeTypeOf(type);
        });
      });
      ...
      describe('array', () => {
        it('generates an array with passed size', () => {
          const randomSize = faker.number.int(1000);
